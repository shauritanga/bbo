...
function Receipt() {
  ...
  const updatePayment = async(selected, status) =>{
    for(let item in selected){
      ...
      const response = await fetch(`http://localhost:3000/api/receipts/${selected[item]._id}`,{
        mode: 'cors',
        method:"POST",
        headers: {
  ...
  const filtered = receipts.filter(expense => expense.payee.toLowerCase().includes(query.toLowerCase()));
  ...
  return (
    <div className='receipt'>
      ...
      <div className="receipt-actions">
          ...
          <div className="receipt-actions_hiden" style={{visibility:visible?"visible":"hidden"}}>
            <button onClick={() => updatePayment(selected,{status:"approved"})}>Approve</button>
            <button onClick={() => updatePayment(selected,{status:""})}>Disapprove</button>
      ...
      <div className="receipt-table">
        <table style={{width:"100%"}}>
          ...
          {openReceiptForm && <ReceiptModal setOpenReceiptForm={setOpenReceiptForm}/>}
    </div>
  )
}
