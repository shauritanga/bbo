...
const ExecutionForm = ({ open, setOpen, orderId, customerId }) => {
  ...
  const handleSubmit = async () => {
    ...
    const { executionResponse, dseResponse } = await Promise.all([
      axios.post(`http://localhost:5001/api/v1/executions`, postData),
      axios.post(`http://localhost:5001/api/v1/dse`, dseData),
      axios.post(`http://localhost:5001/api/v1/vat`, vatData),
      axios.post(`http://localhost:5001/api/v1/cds`, cdsData),
      axios.post(`http://localhost:5001/api/v1/csma`, csmaData),
      axios.post(`http://localhost:5001/api/v1/fidelity`, fidelityData),
      axios.post(`http://localhost:5001/api/v1/brokerage`, brokerageData),
    ]);
